...
```{r}
# -----------------------------------------------------------
# ⚙️ Chargement des librairies nécessaires
# -----------------------------------------------------------
library(ade4)
library(ggplot2)
library(dplyr)
...
# -----------------------------------------------------------
# 🧭 Vérification de la correspondance des lignes
# -----------------------------------------------------------
# Les ID doivent être dans le même ordre
...
# -----------------------------------------------------------
# 📊 1. ACP sur chaque tableau
# -----------------------------------------------------------
# Sélectionne uniquement les colonnes numériques correspondant aux espèces
...
# 🧠 4. Interprétation
# -----------------------------------------------------------
# Coefficient RV (corrélation globale entre structures)
RV <- RV.rtest(pca2001$tab, pca2022$tab)
RV
plot(RV)

# -----------------------------------------------------------
# 💾 5. Sauvegarde des coordonnées si besoin
# -----------------------------------------------------------
...
# Ici la création de la feuille précence et absence pour salix et carex 
...
# On part de ton data.frame déjà chargé
df <- final_salix_herb
...
```{r}
# Table croisée de co-occurrence
...
df %>%
  ...
  labs(x = "Salix herbacea (0=abs, 1=pres)",
       ...
       title = "Co-occurrence des deux espèces")
```
...
ggplot(df, aes(x = X_L93, y = Y_L93, color = interaction(Carex, Salix))) +
  ...
  labs(title = "Distribution spatiale des co-occurrences",
       color = "Présence des espèces")
```

```{r}
cor.test(df$Carex, df$Salix, method = "spearman")  # test de corrélation non paramétrique
...
# ------------------------
# 2️⃣ Test de cooccurrence séparé par année
# ------------------------
...
En 2001
L’odds ratio < 1 (0.63) suggère une tendance à l’exclusion,

👉 mais pas significative statistiquement (p = 0.12).
Autrement dit, en 2001, Salix herbacea et Carex myosuroides pouvaient coexister dans certains plots, sans qu’on détecte un schéma clair d’exclusion ou d’association.

En 2022

L’odds ratio chute à 0.21, et le test devient hautement significatif (p < 0.001).
Cela signifie qu’en 2022, les deux espèces sont nettement moins souvent retrouvées ensemble qu’attendu aléatoirement.
→ Leur co-occurrence a diminué dans le temps.
...
library(tidyverse)
library(ggplot2)
library(patchwork)  # pour mettre deux cartes côte à côte
...
# -------------------------------
# 3️⃣ Séparer les années
# -------------------------------
df_2001 <- df %>% filter(Year == 2001)
df_2022 <- df %>% filter(Year == 2022)

# -------------------------------
# 4️⃣ Créer les cartes
# -------------------------------
carte_2001 <- ggplot(df_2001, aes(x = X_L93, y = Y_L93, color = combo)) +
  ...
  theme_minimal() +
  coord_equal() +
  labs(title = "2001 - Distribution des espèces",
       color = "Présence")
...
carte_2022 <- ggplot(df_2022, aes(x = X_L93, y = Y_L93, color = combo)) +
  ...
  theme_minimal() +
  coord_equal() +
  labs(title = "2022 - Distribution des espèces",
       color = "Présence")

# -------------------------------
# 5️⃣ Afficher côte à côte
# -------------------------------
carte_2001 + carte_2022
```
```{r}
# -------------------------------
# 1️⃣ Packages nécessaires
# -------------------------------
library(tidyverse)
...
df <- final_salix_herb %>%
  ...
  mutate(
    combo = case_when(
      Carex == 1 & Salix == 1 ~ "Les deux espèces",
      Carex == 1 & Salix == 0 ~ "Carex seule",
      Carex == 0 & Salix == 1 ~ "Salix seule",
...
# -------------------------------
# 6️⃣ Assembler les deux cartes
# -------------------------------
(carte_2001 + carte_2022) +
  plot_annotation(
    title = "Distribution spatiale de Carex myosuroides et Salix herbacea",
    subtitle = "Comparaison entre 2001 et 2022 dans l'écosystème alpin de l'Aravo",
    theme = theme(plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
                  plot.subtitle = element_text(size = 13, hjust = 0.5))
  )
```

# Jeu de donnée du CBNA avec Carex at Saule
...
# --- 2. Renommer la colonne X (si nécessaire) ---
...
# --- 9. Résumé rapide ---
cat("\nNombre de relevés Carex post-1990 :", nrow(chk.full))
cat("\nNombre de relevés Salix post-1990 :", nrow(Sal.full))
cat("\nNombre de relevés avec les deux espèces :", sum(Presence.combined$both_species))

# --- 10. Carte des relevés communs ---
# On récupère les coordonnées à partir de chk.SIT (ou Sal.SIT si préférence)

releves_communs_df <- Presence.combined %>%
  filter(both_species == 1) %>%   # 1 = les deux espèces présentes
  left_join(chk.SIT %>% dplyr::select(id_releve, x_l93, y_l93), by = "id_releve")

ggplot(releves_communs_df, aes(x = x_l93, y = y_l93)) +
  geom_point(color = "darkgreen", size = 3) +
  theme_minimal() +
  labs(title = "Relevés communs Carex myosuroides & Salix herbacea",
       x = "Coordonnée X (L93)",
       y = "Coordonnée Y (L93)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))

# --- Optionnel : carte colorée par année ---
releves_communs_df <- Presence.combined %>%
...
ggplot(releves_communs_df, aes(x = x_l93, y = y_l93, color = factor(YR.REL))) +
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "Relevés communs Carex & Salix par année",
       x = "Coordonnée X (L93)",
       y = "Coordonnée Y (L93)",
       color = "Année") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
# Calculer le nombre de relevés communs par année
...
# Graphique du nombre de relevés communs par année
ggplot(evolution, aes(x = YR.REL, y = n_communs)) +
  geom_line(color = "darkgreen") +
  geom_point(color = "darkgreen") +
  theme_minimal() +
  labs(title = "Évolution des relevés communs Carex & Salix par année",
       x = "Année",
       y = "Nombre de relevés communs")
```
...
ggplot(evolution, aes(x = YR.REL, y = prop_communs)) +
  ...
  theme_minimal() +
  labs(title = "Proportion de relevés avec Carex & Salix par année",
       x = "Année",
       y = "Proportion de relevés communs")
```
...
# --- Créer une table avec les années et présence des espèces ---
...
ggplot(evolution_full, aes(x = YR.REL, y = count, color = type)) +
  ...
  scale_color_manual(
    ...
    labels = c("Carex seul", "Salix seul", "Les deux espèces")
  ) +
  labs(
    title = "Évolution des relevés Carex, Salix et communs par année",
    x = "Année",
    y = "Nombre de relevés",
    color = "Type de relevé"
  ) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
```
# Vérification des effectifs par an 

```{r}
library(dplyr)
library(ggplot2)

# --- Vérifier le nombre de relevés par année ---
...
ggplot(releves_par_an, aes(x = YR.REL, y = n)) +
  ...
  theme_minimal() +
  labs(
    title = "Nombre de relevés par année",
    x = "Année",
    y = "Nombre de relevés"
  )
```
...
ggplot(chk.SIT, aes(x = x_l93, y = y_l93, color = as.factor(YR.REL))) +
  geom_point() +
  theme_minimal() +
  labs(title = "Répartition spatiale des relevés au fil du temps",
       x = "X L93", y = "Y L93", color = "Année")
```
...
# --- 1. Ajouter altitudes, années ET coordonnées depuis chk.SIT (Carex) et Sal.SIT (Salix) ---
...
cat("✅ Fusion terminée :", nrow(Presence.combined), "relevés\n")
cat("Colonnes disponibles :\n")
print(names(Presence.combined))

# --- 4. Enregistrement du fichier CSV ---
# Détecte automatiquement le répertoire où se trouve ton script
...
cat("💾 Fichier enregistré à l'emplacement suivant :\n", output_path, "\n")
...
# --- 1. Identifier les id communs entre les deux jeux de données ---
intersect_ids <- intersect(chk.SIT$id_releve, Sal.SIT$id_releve)
cat("Nombre de relevés communs :", length(intersect_ids), "\n")


# --- Redéfinir les sélections avec dplyr::select() explicite pour éviter les conflits ---
...
cat("Différences d'altitude :", nb_diff_elev, "\n")
cat("Différences d'année :", nb_diff_year, "\n")
...
# --- 2. Création du jeu de données pour prédictions ---
...
ggplot(pred_df, aes(x = ELEV, y = Probability, color = Species)) +
  ...
  labs(
    title = "Effet de l'altitude sur la probabilité de présence",
    subtitle = "Modèle logistique binomial",
    x = "Altitude (m)",
